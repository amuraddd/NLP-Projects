...
def predict_new(dataframe):
    ...
    #predict from models
    logit_predict_probs = logit.predict_proba(scaled_data)[:,1]    
    logit_predict_binary = np.array([1 if y>0.4 else 0 for y in logit_predict_probs])
    
    rf_predict_probs = rf.predict_proba(scaled_data)[:,1]
    rf_predict_binary = np.array([1 if y>0.4 else 0 for y in rf_predict_probs])
    
    #convert to dfs
    lasso_predict_probs = lasso.predict(scaled_data)
    lasso_predict_binary = np.array([1 if y>0.4 else 0 for y in lasso_predict_probs])
    
    logit_df = pd.DataFrame(logit_predict_binary, columns=['target'], index=dataframe.index)
    logit_df.index.name='id'
    
    rf_df = pd.DataFrame(rf_predict_binary, columns=['target'], index=dataframe.index)
    rf_df.index.name='id'
    
    lasso_df = pd.DataFrame(lasso_predict_binary, columns=['target'], index=dataframe.index)
    lasso_df.index.name='id'
    
    return logit_df, rf_df, lasso_df

#=================================Test model and predict on unknown data===================================
#model performance
predict(data)  

#unknown data
logit, rf, lasso = predict_new(data)

logit.to_csv('test/logit_test.csv')
rf.to_csv('test/rf_test.csv')
lasso.to_csv('test/lasso_test.csv')